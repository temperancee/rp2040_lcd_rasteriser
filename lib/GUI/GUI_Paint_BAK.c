...
void Paint_NewImage(uint8_t *image, uint16_t Width, uint16_t Height, uint16_t Rotate, uint16_t Color)
{
    ...
    // By default, Paint.Scale is 2, so each byte in image stores 8 pixels
    // where the colour of each is white if 1, black if 0 (or vice versa).
    // Given that, the width of the image in bytes is either exactly
    // width Div 8, or, if the width is not divisible by 8 and overruns,
    // it is width div 8 + 1.
    // For example, if an image is 122 pixels, then 122 div 8 = 15 bytes
    // will not suffice, as we lose the last 2 pixels. Instead, we use
    // 16 bytes, and ignore the last 6 bits of the final byte.
    // Note, Paint.WidthByte is a uint16_t, which you may think is a waste here,
    // but for larger scales (65k) we make use of the extra byte.
    Paint.WidthByte = (Width % 8 == 0)? (Width >> 3 ): ((Width >> 3) + 1);
...
void Paint_SetScale(uint8_t scale)
{
    if(scale == 2){
        ...
        Paint.Scale = scale;
        // There are 4 possible colours, so we need 2 bits for each pixel, thus
        // one byte stores 4 pixels now, rather than 8 in the Scale=2 case.
        // Thus, we check if WidthMemory is divisible by 4. Then WidthMemory/4
        // is enough bytes to store a row of the image. If not, we need an extra
        // byte to store the remaining pixels, so we add 1.
        Paint.WidthByte = (Paint.WidthMemory % 4 == 0)? (Paint.WidthMemory / 4 ): (Paint.WidthMemory / 4 + 1);
    }else if(scale ==16) {
        Paint.Scale = scale;
        // As above, but now we need 4 bits per pixel, so a byte only stores 2 pixels
        Paint.WidthByte = (Paint.WidthMemory%2==0) ? (Paint.WidthMemory/2) : (Paint.WidthMemory/2+1); 
    }else if(scale ==65) {
        Paint.Scale = scale;
        // Now the number of bytes needed is simply double WidthMemory, because
        // each pixel has 16 bits of colour, so each pixel is 2 bytes.
...
{
    // WARNING: There was originally a bug in Paint.Scale == 65 block
    // using Paint.WidthByte instead of Paint.Width for the X loop
    // upper bound. This might be a problem for the other Scales,
    // so be careful - it could lead to overwritting random
    // parts of memory!!!!
    if(Paint.Scale == 2 || Paint.Scale == 4) {
        ...
        for (uint16_t Y = 0; Y < Paint.HeightByte; Y++) {
            for (uint16_t X = 0; X < Paint.Width; X++ ) {//8 pixel =  1 byte
                uint32_t Addr = X*2 + Y*Paint.WidthByte;
                // These bit masks may appear pointless, but they
                // truncate the color from 16 bit to 8 bit
                Paint.Image[Addr] = 0xff & (Color>>8);
    ...
    for(j = 0; j < H_Image; j++){
        for(i = 0; i < W_Image; i++){
            if(xStart+i < Paint.WidthMemory  &&  yStart+j < Paint.HeightMemory)//Exceeded part does not display
                // Store Color in little endian, otherwise it gets inverted later
